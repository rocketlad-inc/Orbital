...
export function computeTrajectory(
  ...
  for (let arcCount = 0; arcCount < TRAJ_MAX_ARCS && tCursor < tEnd; arcCount++) {
    ...
    if (event) {
      ...
      // Always push the arc up to the node — even if the node has
      // dv=0. Skipping was too aggressive: it left the trajectory
      // with NO arc containing the current tick, which broke the
      // KSP-style ellipse rendering (the renderer drew the orbit at
      // arc.tStart's parent position instead of the live parent
      // position). Drawing-side dedup handles the overlap visually.
      arcs.push({
        orbit: currentOrbit,
        tStart: tCursor,
        tEnd: nextNode.t,
