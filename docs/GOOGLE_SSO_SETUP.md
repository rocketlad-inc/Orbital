...
`migrations/0014_google_oauth.sql` adds:
- `users.google_sub TEXT UNIQUE` — the canonical Google account ID
- relaxes `users.password_hash` to nullable (Google-only accounts have no password)
...
3. Three branches in upsert order:
   ...
   - **New user**: provisions a row with `password_hash = NULL` and `google_sub`.
...
- A user who originally signed in via Google has `password_hash = NULL`.
